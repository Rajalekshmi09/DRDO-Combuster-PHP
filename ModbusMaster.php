...
class ModbusMaster
{
  ...
  public $status;
  public $timeout_sec = 30; // Timeout 30 sec
  ...
  private function writeMultipleRegisterPacketBuilder($unitId, $reference, $data, $dataTypes)
  {
    ...
    foreach ($data as $key => $dataitem) {
      if  ($dataTypes[$key] == "INT" || $dataTypes[$key] == "UINT"  ) {  //modified by raji INT to UINT
    ...
    // foreach ($data as $key => $dataitem) {
    //   $dataType = array_key_exists($key, $dataTypes) ? $dataTypes[$key] : "UINT";

    //   if  ($dataType == "INT" || $dataType == "UINT"  ) { 
    //   // if  ($dataTypes[$key] == "INT" || $dataTypes[$key] == "UINT"  ) {  //modified by raji INT to UINT
    //     $buffer1 .= iecType::iecINT($dataitem);   // register values x
    //     $dataLen += 2;
    //   } elseif ($dataType == "DINT") {
    //   // } elseif ($dataTypes[$key] == "DINT") {
    //     $buffer1 .= iecType::iecDINT($dataitem, $this->endianness);   // register values x
    //     $dataLen += 4;
